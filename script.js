...
/* ------------------------------------------------------------
   2. Preloader → cinematic page-load sequence
   ------------------------------------------------------------ */
...
/* ------------------------------------------------------------
   3. Animated counters (hero statistics)
   ------------------------------------------------------------ */
...
/* ------------------------------------------------------------
   9. Master rAF loop — cursor, spotlight, parallax, progress,
      aurora morph. One loop, transform-only writes.
   ------------------------------------------------------------ */
...
function masterLoop() {
    ...
    // --- Custom cursor + spotlight (lerped trailing) ---
    if (FINE_POINTER && !REDUCED) {
    ...
    document.documentElement.style.setProperty('--aurora-hue', `${pct * 45}deg`);
...
if (canvas && !REDUCED) {
    ...
    function drawParticles(t) {
        ...
        if (rafActive) requestAnimationFrame(drawParticles);
    }

    sizeCanvas();
    buildParticles();
    requestAnimationFrame(drawParticles);
    resumeParticles = () => requestAnimationFrame(drawParticles);
...
document.addEventListener('visibilitychange', () => {
    const wasActive = rafActive;
    rafActive = document.visibilityState === 'visible';
    if (rafActive && !wasActive) {
        requestAnimationFrame(masterLoop);
        resumeParticles();
    }
});
...
/* ------------------------------------------------------------
   11. Navbar scrolled state (rAF-free, cheap)
   ------------------------------------------------------------ */
...
/* ------------------------------------------------------------
   12. Project modals — spring glass sheets
   ------------------------------------------------------------ */
function initModals() {
...
/* ------------------------------------------------------------
   13. Contact form — EmailJS with graceful button states
   ------------------------------------------------------------ */
const contactForm = document.getElementById('contactForm');

window.addEventListener('load', () => {
    if (typeof emailjs !== 'undefined') emailjs.init('4KiOy1BEpD-goxZEn');
});
